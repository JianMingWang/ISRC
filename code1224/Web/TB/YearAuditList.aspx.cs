...
{
    public partial class YearAuditList : PageBase
    {
        ...
        protected void ddlCycle_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            PublicMethod.cycleList(ddlStartPeriod, ddlCycle.SelectedValue.ToString().Trim());
            PublicMethod.cycleList(ddlEndPeriod, ddlCycle.SelectedValue.ToString().Trim());
            PublicMethod.SetDdlSelected(index, month, ddlStartPeriod);
            PublicMethod.SetDdlSelected(index, month, ddlEndPeriod);
            ddlStartPeriod.Label = "";
            ddlEndPeriod.Label = "";
        }
        ...
        {
            string strWhere = string.Format(" Status!='0' ORDER BY FillDate DESC ");
        ...
        protected void gridAudit_RowClick(object sender, GridRowClickEventArgs e)
        {
            Session["selectedReportID"] = gridAudit.Rows[e.RowIndex].DataKeys[0].ToString();
            LoadData(Session["selectedReportID"].ToString());
        }

        protected void gridAudit_RowCommand(object sender, GridCommandEventArgs e)
        {
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            string startYear = nbxStartYear.Text.Trim();
            string endYear = nbxEndYear.Text.Trim();
            string startPeriod = "", endPeriod = "";
            if (cycle != "4" && cycle != "0")
            {
                startPeriod = ddlStartPeriod.SelectedValue.ToString().Trim();
                endPeriod = ddlEndPeriod.SelectedValue.ToString().Trim();
            }
            string status = ddlStatus.SelectedValue.ToString().Trim();
            BLL.vw_Report_Dept_Account bllReport = new BLL.vw_Report_Dept_Account();
            DataSet dsReport = bllReport.SearchReportList(cycle, startYear, startPeriod, endYear, endPeriod, status);
            DataTable dtSource = dsReport.Tables[0];
